<?php
  ...
  $idconduce =  base64_decode(isset($_GET['conduce']) ? $_GET['conduce'] : '');
  ...
  spl_autoload_register(function($className){
    ...
    $listado = $objconduce->Listar_Detalle($idconduce);

    $param_moneda = $objconduce->Ver_Moneda_Reporte();
    foreach ($param_moneda as $row => $column) {
        $moneda = $column['CurrencyName'];
    }

    $info = $objconduce->Listar_Info($idconduce);

    if (is_array($info) || is_object($info)){
    	foreach ($info as $row => $column) {
    		$numero_conduce = $column["numero_conduce"];
    		$fecha_conduce = $column["fecha_conduce"];
        $fecha_conduce = DateTime::createFromFormat('Y-m-d H:i:s', $fecha_conduce)->format('d/m/Y H:i:s');
    ...
    $pdf->Cell(46,8,'NO. CONDUCE : ');
    $pdf->SetFont('Arial','B',14);
    $pdf->Cell(37,8,$numero_conduce);
    ...
    if (is_array($listado) || is_object($listado))
    {
      ...
      $pdf->Output('I','conduce'.$numero_conduce.'.pdf');

  } catch (Exception $e) {

    $pdf->Text(22.8, 5, 'ERROR AL IMPRIMIR CONDUCE');
    $pdf->Output('I','CONDUCE_ERROR.pdf',true);
